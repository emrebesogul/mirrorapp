...
export default class WidgetDeleteZone extends React.Component {
    ...
    render() {
        return <View style={[this.props.style, this.state.active ? {backgroundColor: 'red'} : null]} pointerEvents={this.props.pointerEvents}
                     onLayout={this.reportMeasurements} ref={c => this.wrapper = c}>
            {
                React.Children.map(this.props.children, child => {
                    return React.cloneElement(child, Object.assign({}, this.props.children.props));
                })
            }
        </View>
